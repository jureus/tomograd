...
?>
<div class='footer__feedback' id="<?=$randId?>">
        <div class='container'>

            <div class='footer__feedback__block'>
                <div class='footer__feedback__bg'></div>
                <div class='footer__feedback__content'>
                    <div class='footer__feedback__title'><?=$arParams["TITLE"]?></div>
                    <div class='footer__feedback__text'>
                        <p><?=$description?></p>
                    </div>
                    <div class='footer__feedback__form'>
...
<script>
    ...
    const section<?=$randId?> = document.getElementById('<?=$randId?>');
    // Находим форму внутри этой секции
    const form<?=$randId?> = section<?=$randId?>.querySelector("form[name='<?=$arResult["arForm"]["SID"]?>']");

    // Назначаем обработчик отправки
    form<?=$randId?>.addEventListener("submit", function(e) {
        e.preventDefault();
        let isValid = true;

        // Очистка предыдущих ошибок только внутри этой формы
        form<?=$randId?>.querySelectorAll(".form__input").forEach(field => {
        ...
        form<?=$randId?>.querySelectorAll(".form__input__text").forEach(input => {
        ...
        if (isValid) {
            const formData = new FormData(form<?=$randId?>);
