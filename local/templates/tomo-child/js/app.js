...
if (psInput) {
    ...
    psInput.onblur = function() {
        if (psInput.value.length) {
            ...
            psClear.classList.remove('active')
            psInput.classList.remove('active')
        }
    }
...
if (navBtns) {
    navBtns.forEach(navBtn => {
        navBtn.addEventListener('click', () => {
            ...
            if (navheader.dataset.top === "true") {
                let hcatalog = document.querySelector('.js--hcatalog');
                if (hcatalog) {
                    if (!hcatalog.classList.contains('active')) {
    ...
    // overlay mobile menu
    const overlayMobileMenu = document.querySelector('.js--mobilemenu-overlay');
    if (overlayMobileMenu) {
        overlayMobileMenu.addEventListener('click', function() {
            nav.classList.remove('mobilemenu__opened')
            navBtn.classList.remove('active')
            document.body.classList.toggle('no-scroll')
        })
...
;
    ...
    // детское отделение
    const sliderCHWelcome = document.querySelector('.js--sl-chwelcome');

if(sliderCHWelcome) {
    const CHwelcomeSwiper = new Swiper(sliderCHWelcome, {
        loop: true,
        slidesPerView: 1,
        autoHeight: true,
        effect: 'fade',
        ...
        pagination: {
            el: '.js--sl-chwelcome-pag',
            clickable: true,
            bulletClass: 'ch-welcome__pag__bullet',
            bulletActiveClass: 'active'
        },

        navigation: {
            disabledClass: 'disabled',
            prevEl: '.js--sl-chwelcome-prev',
            nextEl: '.js--sl-chwelcome-next',
        },
    })
}
;
    const sliderChadvantages = document.querySelector('.js--sl-chadvantages');

if (sliderChadvantages) {
    const sliderChadvantagesOffset = document.querySelector('.js--sl-chadvantages__offset');

    let leftchadv = function leftchadv() {
        let offsetLeft;
        return offsetLeft = sliderChadvantagesOffset.offsetLeft + getScrollbarWidth()/2;
    };

    let swiperChadvantages = new Swiper(sliderChadvantages, {
        init: false,
        slidesPerView: 'auto',
        freeMode: true,
        autoHeight: true,
        speed: 1000,
        slidesOffsetBefore: leftchadv(),
        slidesOffsetAfter: leftchadv(),
        spaceBetween: 13,
        slideToClickedSlide: false,
        breakpoints: {
            992: {
    ...
    function blogInitSwiper() {
        if (window.innerWidth < 768) {
            ...
            swiperChadvantages.init();
            swiperChadvantages.slideTo(0);

            window.addEventListener('resize', () => {
                swiperChadvantages.params.slidesOffsetBefore = leftchadv();
                swiperChadvantages.params.slidesOffsetAfter = leftchadv();
                swiperChadvantages.update(true);
                if (swiperChadvantages.activeIndex === 0) {
                    swiperChadvantages.slideTo(0);
                }
            })
            ...
            if (swiperChadvantages.isInit) {
                swiperChadvantages.destroy();
            }
        }
    }
}
;
    const sliderCHBanner2 = document.querySelector('.js--sl-chbanner2');

if(sliderCHBanner2) {
    const CHbanner2Swiper = new Swiper(sliderCHBanner2, {
        loop: true,
        slidesPerView: 1,
        autoHeight: true,
        effect: 'fade',
...
;
    ...
    // ch tabs
    const chtabs = document.querySelector('.js--chtabs');
    if (chtabs) {
        const chbtntabs = chtabs.querySelectorAll('.js--chtabs-btn');
        const chtabsWrap = document.querySelector('.js--chtabs-wrap');

        chbtntabs.forEach(btn => {
            btn.addEventListener('click', function(e) {
                e.preventDefault();

                const prevActiveItem = chtabs.querySelector('.js--chtabs-btn.active');
                const prevActiveButton = chtabs.querySelector('.js--chtabs-item.active');
                ...
                const nextActiveItem = chtabs.querySelector(nextActiveItemId);

                btn.classList.add('active');
                nextActiveItem.classList.add('active');
                chtabsWrap.classList.toggle('active');
            })
        })
    }

    // эффект движения картинок от мышки в форме
    const wrapeffect = document.querySelector('.js--effect');
    if (wrapeffect && window.innerWidth > 991) {
        wrapeffect.addEventListener('mousemove', function(e) {
            let pics = wrapeffect.querySelectorAll('.js--effect-bg');
            pics.forEach((move) => {
    ...
    // анимация шариков от мышки в баннере
    const root = document.querySelector('.js--chbanner')
    if (root) {
        let oldX = 0,
            oldY = 0,
            deltaX = 0,
        ...
        root.querySelectorAll('.js--chbanner-pic').forEach(el => {
    ...
    // styled Scroll form blog
    const blogScroll = document.getElementById('js--styled-scrollblog');
    let initscroll = false;

    function scrollBlog(initscroll) {
        if (initscroll) {
            new SimpleBar(document.getElementById('js--styled-scrollblog'), {
                autoHide: false,
            })
        }
    }
    if (blogScroll && window.innerWidth > 991) {
        initscroll = true;
        scrollBlog(initscroll)
    } else {
        initscroll = false;
    }
})
    ...
    function init() {
        ...
        placemarks.forEach((placemark, index) => {
            let mark = new ymaps.Placemark(placemark.coords, {
                ...
                iconImageHref: '/local/templates/tomo-child/img/ic-map-pin.png',
                iconImageSize: [38, 28],
