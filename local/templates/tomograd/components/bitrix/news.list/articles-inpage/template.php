...
foreach ($arResult["ITEMS"] as $key => $item) {
	...
	if (empty($item["PROPERTIES"]["LINKED_SECTION"]["VALUE"])) {
		unset($arResult["ITEMS"][$key]);
	} 

	foreach ($item["PROPERTIES"]["LINKED_SECTION"]["VALUE"] as $id) {
		$unset = true;
		if ($id == $linked_el_id) {
			$unset = false;
		}

		if ($unset) unset($arResult["ITEMS"][$key]);
	}
}
?>
			...
			<div class="actionarticle__procedures__item col-6" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
				<div class="actionarticle__procedures__card color-<?=$bg_colors[$arItem["PROPERTIES"]["BG_COLOR"]["VALUE"]]?>">
					...
					<div class="actionarticle__procedures__card__text">
						<div class="actionarticle__procedures__card__title"><?=$arItem["NAME"]?>:</div>
						<div class="actionarticle__procedures__card__text"><?=$arItem["PREVIEW_TEXT"]?></div>
					</div>
				</div>
			</div>
			<?endforeach;?>
